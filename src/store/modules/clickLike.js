...
import reqModule from '@/assets/js/api/reqModule';
const state = () => ({
  app: []
})
// arr1.map(i => {
...
const mutations = {
  ADD_CLICK_DATA: (state, data) => { //data 类型为 {title:'app',belong:'',id:''}
    ...
    const app = Object.assign([],state.app)
    state.app.push(pushData)
    ...
    //     app[index][1].push(data.belong)
    //     state.app = [...app,pushData]
    //   }
    // } else {
    //   app.push(pushData)
    //   state.app = [...app,pushData]
    // }
    // localStorage.setItem('clickLike',JSON.stringify(app))
  }
}

const actions = {
  addData({ dispatch, commit,state }, data) {
    console.log(data)
