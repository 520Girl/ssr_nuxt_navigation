<template>
    <div class="articless-container contents-bck" id="articless">
        <section class="articles-content mx-15-2">
            <Row type='flex' justify="space-between" class-name="articles-carousel" >
                <Col :xl="{ span: 7 }" :lg="{span: 11}" :xs="{ span: 24 }" class-name="carousel-content">
                    <Carousel v-model="carousel.pitchOnImg" :autoplay="true" radius-dot style="height: 100%">
                        <CarouselItem  v-for="(item,index) in carousel.allCarousel" style="height: 100%" :key="index">
                            <div  @click="jumpUrl(item)" class="carousel" v-click-data="{name:item.title,hrefUrl:`${item.belong}/detailed?id=${item.belongId}`,imgUrl:'/static/images/article' + item.imgUrl}" >
                                <img v-lazy="'/static/images/article' + item.imgUrl" data-article="article1" :alt="item.title" width="518" height="300">
                                <div class="text applyBck" :data-index="index" >
                                    <h5 class="imgInIcon fontSize-text-colornoH">{{item.title}}</h5>
                                </div>
                            </div>
                        </CarouselItem>
                    </Carousel>
                </Col>
                <Col :xl="{ span: 3 }"  :lg="{span: 3}" :xs="{ span:0 }" class-name="sliders pdt-1-0">
                    <div class="sliders-layout">
                        <a href="javascript:void(0)"  @click="jumpUrl(item)" v-for="(item,index) in carousel.allSliders "  :key="index" v-click-data="{name:item.title,hrefUrl:`${item.belong}/detailed?id=${item.belongId}`,imgUrl:'/static/images/article' + item.imgUrl}">
                            <h5 class="overflow-eclipse applyBck fontSize-text-colornoH">{{item.imgAlt}}</h5>
                            <img v-lazy="'/static/images/article'+ item.imgUrl" data-article="article2" :alt="item.imgAlt" width="202"  height="89">
                        </a>
                    </div>
                </Col>
                <Col :xl="{ span: 7 }"  :lg="{span: 0}" :xs="{ span:0 }" class-name="card pdt-1-0">
                    <Card bordered >
                        <p slot="title" class="card-title fontSize-text-color ">
                            <Icon type="ios-stats fontSize-icon"></Icon>
                            排行榜
                        </p>
                        <a href="#" slot="extra" @click.prevent="changeLimit" class="card-change">
                            更多
                            <Icon type="ios-arrow-forward"></Icon>

                        </a>
                        <ul class="card-main">
                            <li v-for="(item,index) in rankingList" class="main-item" :key="index" @mouseenter="mainItem = index">
                                <Icon type="md-star"  class="main-circle" :style="index == mainItem ? 'color:#f5a623;font-size:15px; ' : ''" v-if="index == 0"/>
                                <Icon type="md-radio-button-on" class="main-circle" :style="index == mainItem ? 'color:#f5a623;font-size:14px;' : 'font-size:12px;left:-15px; '"  v-else/>
                                <i class="main-line main-line-color"></i>
                                <div class="item-text">
                                    <a :href="item.url" target="_blank">
                                        {{ item.name }}
                                        <img :src="item.favicon" alt="" v-if="item.favicon">
                                        <Icon type="ios-stats-outline" v-else/>
                                    </a>
                                    <span>
                                        <Rate show-text allow-half disabled :count="5"  v-model="item.rate" >
                                            <span class="fontSize-text-color">{{item.rate}}</span>
                                        </Rate>
                                    </span>
                                </div>
                                <div class="item-img" v-show="index == mainItem">
                                    <a>
                                        <img v-lazy="'https://huyaimg.msstatic.com/cdnimage/upcoming/up_16006687034205_pic.jpg'" data-article="article3" title="预约">
                                    </a>
                                    <a >
                                        <span></span>
                                        <span>预约</span>
                                    </a>
                                </div>
                            </li>
                        </ul>
                    </Card>
                </Col>
                <Col :xl="{ span: 7 }" :lg="{span: 10}"  :xs="{ span: 24 }" class-name="collapse pdt-1-0">
                    <Collapse v-model="collapse.pitchOnTag" >
                        <Panel :name="String(index)" v-for="(item,index) in collapse.allCollapse" :key="index">
                            <span class="fontSize-text-color">{{item.title}}</span>
                            <Badge :count="item.heartNum" overflow-count="99" color="magenta"  show-zero ></Badge>
                            <p slot="content" class="fontSize-text-color">
                                {{item.explain}}
                            </p>
                            <span class="sign" >
                                <router-link :to="{path:`/blog/detailed?id=${item._id}`,query:{title:item.title}}" v-click-data="{name:item.title,hrefUrl:`/blog/detailed?id=${item._id}`,imgUrl:'/static/images/blog' + item.imgUrl}" class="bottom fontSize-text-orange" >
                                    <sub class="left-sign">
                                        <Time :time="item.onlineTime" type="date" v-if="item.onlineTime"></Time>
                                    </sub>
                                    <sub class="right-sign">
                                        {{item.author}}
                                    </sub>
                                </router-link>
                                <router-link tag="a" :to="{path:'/blog'}" class="top" v-if="index == 0" >
                                    <sup class="top-sign">
                                    所有
                                    </sup>
                                </router-link>
                            </span>
                        </Panel>
                    </Collapse>
                </Col>
            </Row>
        </section>
    </div>
</template>
<script>
  // import {Row,Col,Collapse,Panel,Carousel,CarouselItem,Badge,Card,Icon,Rate,Time   } from 'iview';
  export default {
    name:"articles",
    // components:{Row,Col,Collapse,Panel,Carousel,CarouselItem,Badge,Card,Icon,Rate,Time    },
    filters:{
      imgUrlFilters(url) {
        if (/\.(jpg|png|gif|jpeg|svg)$/.test(url) && /^(http|https)/.test(url)) {
          return url
        } else if (/\.(jpg|png|gif|jpeg)$/.test(url)) {
          return 123
        }
        return 123
      },
    },
    data() {
      return {
        //博客数据
        collapse:{
          pitchOnTag:'0',
          allCollapse:[
            {id:1,heartNum:12,title:'小小鸟',explain:'我是一只小小鸟，怎么飞也飞不高，'},
            {id:1,heartNum:14,title:'飞鹏',explain:'我是一只小小鸟，怎么飞也飞不高，'},
            {id:1,heartNum:12,title:'小小鸟',explain:'我是一只小小鸟，怎么飞也飞不高，'},
          ],
        },
        //轮播图
        carousel:{
          pitchOnImg: 0,
          allCarousel:[
            // {id:1,imgHref:"www.baidu.com",imgUrl:'https://i2.wp.com/se-zhan.xyz/wp-content/uploads/2020/09/20200914084541.png?fit=851%2C489&ssl=1',title:'易用vpn',imgWidth:518,imgHeight:300},
            // {id:2,imgHref:"www.baidu.com",imgUrl:'https://i1.wp.com/www.ymrkou.com/wp-content/uploads/2020/08/d3f513902a5b1471d55d3c7d9d2b508d.gif?fit=960%2C540&ssl=1',title:'另一个应用商城',imgWidth:518,imgHeight:300},
          ],
          allSliders:[
            {id:1,imgHref:"www.baidu.com",imgUrl:'https://i2.wp.com/se-zhan.xyz/wp-content/uploads/2020/09/20200914084541.png',imgAlt:'易用vpn',imgWidth:202,imgHeight:89},
            {id:1,imgHref:"www.baidu.com",imgUrl:'https://i2.wp.com/se-zhan.xyz/wp-content/uploads/2020/09/20200914084541.png',imgAlt:'易用vpn',imgWidth:202,imgHeight:89},
            {id:1,imgHref:"www.baidu.com",imgUrl:'https://i2.wp.com/se-zhan.xyz/wp-content/uploads/2020/09/20200914084541.png',imgAlt:'易用vpn',imgWidth:202,imgHeight:89},
          ]
        }, //轮播图数据
        number: '',
        mainItem:0,//鼠标经过排行榜
        rankingList: [
          {
            name: '易用vpn',
            url: 'https://movie.douban.com/subject/1292052/',
            rate: 4.9,
            favicon:'data:image/jpeg;base64,'
          },{
            name: '悠悠国际象棋',
            url: 'https://movie.douban.com/subject/1295644/',
            rate: 4.4
          },{
            name:'中央新闻',
            url:'https://www.baidu.com',
            rate:2
          }
        ], //排行榜数据
        time2: (new Date()).getTime() - 86400 * 300 * 1000,

      }
    },
    created() {
      this.getSlide()
      this.getBlogLists()
    },
    methods:{
      // 获取轮播图
      getSlide() {
        this.$api.slide.getSlide({pre_page:5}).then(res=>{
          if (res.code === 200){
            this.carousel.allCarousel = res.slide.content
          }
        })
      },
      // 获取博客数据
      getBlogLists(){
          this.$api.blog.getBolgLists(  {page:1,per_page:5,tag:'',order:-1}).then(res=>{
            if (res.status === 1){
              this.collapse.allCollapse = res.blog.content
            }
          })
      },
      // 跳转页面
      jumpUrl(item){
        if (item.hrefUrl){
          this.$common.goto(item.hrefUrl)
          return
        }else {
          this.$router.push({path:`${item.belong}/detailed`,query:{id:item.belongId,title:item.title}})
          return
        }
      }
    }
  }
</script>
<style lang="scss" scoped>
    @import "~/assets/css/components/index/articles.scss";
    @import "~/assets/css/animation.scss";
</style>
